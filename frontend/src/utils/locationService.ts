...
const browserCurrentPosition = async (): Promise<JoinLocationResult> => {
  if (typeof navigator === 'undefined' || !navigator.geolocation) {
    developmentLog('Geolocation failed:', 'navigator.geolocation is unavailable');
    return {
      status: 'failure',
      reason: 'unsupported',
  ...
  try {
    ...
    if (permission?.state === 'denied') {
      ...
      developmentLog('Geolocation failed:', 'permission denied before request');
      return {
        status: 'failure',
  ...
  developmentLog('Geolocation requested');
        ...
        const reason = error.code === error.PERMISSION_DENIED
          ...
          : error.code === error.POSITION_UNAVAILABLE
            ...
            : error.code === error.TIMEOUT
              ? 'timeout'
              : 'unknown';
        developmentLog('Geolocation failed:', { code: error.code, message: error.message });
        resolve({
          status: 'failure',
          reason,
...
export const getCurrentJoinLocationResult = async (options?: {
  ...
  if (permissionDeniedThisSession && !options?.retryDenied) {
    developmentLog('Geolocation failed:', 'permission was already denied this session');
    return { status: 'failure', reason: 'permission-denied' };
  }
  ...
  const pendingRequest: Promise<JoinLocationResult> = (async (): Promise<JoinLocationResult> => {
    ...
    if (!isValidCoordinate(result.coordinate)) {
      developmentLog('Geolocation failed:', 'browser returned invalid coordinates');
      return {
        status: 'failure',
        reason: 'unknown',
